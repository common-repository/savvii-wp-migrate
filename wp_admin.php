...
class SVWPAdmin {
	...
	function __construct($settings, $siteinfo, $bvapi = null) {
		...
		$this->bvapi = $bvapi;
		$this->bvinfo = new SVInfo($this->settings);
	}
	...
	public function settingsLink($links, $file) {
		if ( $file == plugin_basename( dirname(__FILE__).'/savvii.php' ) ) {
			$links[] = '<a href="'.$this->mainUrl().'">'.__( 'Settings' ).'</a>';
		}
		return $links;
	}

	public function svsecAdminMenu($hook) {
		if ($hook === 'toplevel_page_savvii') {
			wp_register_style('form-styles', plugins_url('css/form-styles.css', __FILE__));
			wp_enqueue_style('form-styles');
			wp_register_style('bvgrid', plugins_url('css/bvgrid.css', __FILE__));
			wp_enqueue_style('bvgrid');
		}
	}
	...
	public function siteInfoTags() {
		...
		$secret = SVRecover::defaultSecret($this->settings);
		$public = SVAccount::getApiPublicKey($this->settings);
	...
	public function activateWarning() {
		global $hook_suffix;
		if (!SVAccount::isConfigured($this->settings) && $hook_suffix == 'index.php' ) {
?>
			<div id="message" class="updated" style="padding: 8px; font-size: 16px; background-color: #dff0d8">
						<a class="button-primary" href="<?php echo esc_url($this->mainUrl()); ?>">Activate Savvii Wp Migrate</a>
						&nbsp;&nbsp;&nbsp;<b>Almost Done:</b> Activate your Savvii account to migrate your site.
			</div>
<?php
		}
	}
