	...
	class BVCallbackRequest {
		...
		public function __construct($account, $in_params, $settings) {
			...
			$this->pubkey_name = !empty($in_params['pubkeyname']) ?
					SVAccount::sanitizeKey($in_params['pubkeyname']) : 'm_public';
			$this->bvprmsmac = !empty($in_params['bvprmsmac']) ? SVAccount::sanitizeKey($in_params['bvprmsmac']) : "";
		...
		public function authFailedResp() {
			$api_public_key = SVAccount::getApiPublicKey($this->settings);
			$default_secret = SVRecover::getDefaultSecret($this->settings);
			$default_account_pubkey = SVAccount::getDefaultPublicKey();
			$bvinfo = new SVInfo($this->settings);
			$resp = array(
				"request_info" => $this->info(),
				"bvinfo" => $bvinfo->info(),
