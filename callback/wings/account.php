...
class BVAccountCallback extends BVCallbackBase {
	...
	function updateInfo($args) {
		...
		if (array_key_exists('update_info', $args)) {
			...
			$result['update_info'] = array(
				"status" => SVAccount::exists($this->settings, $args['update_info']['pubkey'])
			);
		}
	
		if (array_key_exists('update_api_key', $args)) {
			SVAccount::updateApiPublicKey($this->settings, $args['update_api_key']['pubkey']);
			$result['update_api_key'] = array(
				"status" => $this->settings->getOption(SVAccount::$api_public_key)
			);
		}
	...
	function process($request) {
		...
		case "addacc":
			SVAccount::addAccount($this->settings, $params['public'], $params['secret']);
			$resp = array("status" => SVAccount::exists($this->settings, $params['public']));
			break;
		case "rmacc":
			$resp = array("status" => SVAccount::remove($this->settings, $params['public']));
			break;
		case "updt":
			$account->updateInfo($params);
			$resp = array("status" => SVAccount::exists($this->settings, $params['pubkey']));
			break;
		case "updtapikey":
			SVAccount::updateApiPublicKey($this->settings, $params['pubkey']);
			$resp = array("status" => $this->settings->getOption(SVAccount::$api_public_key));
			break;
		case "rmbvscrt":
		...
		case "rmoldbvacc":
			...
			break;
		case "fetch":
			$accounts = SVAccount::allAccounts($this->settings);
