...
if (!class_exists('SVInfo')) :
	class SVInfo {
		public $settings;
		public $config;
		public $plugname = 'savvii';
		public $brandname = 'Savvii Migrate';
		public $badgeinfo = 'svbadge';
		public $ip_header_option = 'svipheader';
		public $brand_option = 'svbrand';
		public $version = '5.56';
		public $webpage = 'http://www.savvii.com';
		public $appurl = 'https://migrate.blogvault.net';
		public $slug = 'savvii-wp-migrate/savvii.php';
		public $plug_redirect = 'svredirect';
		public $logo = '../img/savvii-logo.png';
		public $brand_icon = '/img/icon.png';
		public $services_option_name = 'BVSERVICESOPTIONNAME';
		public $author = 'Savvii';
		public $title = 'Savvii WP Migrate';
		...
		public function hasValidDBVersion() {
			return SVInfo::DB_VERSION === $this->getCurrentDBVersion();
		}
		...
		public function getConnectionKey() {
			...
			return base64_encode(SVRecover::defaultSecret($this->settings).":".$bvsiteinfo->siteurl());
		}

		public function getDefaultSecret() {
			require_once dirname( __FILE__ ) . '/recover.php';
			$bvsiteinfo = new SVWPSiteInfo();
			return SVRecover::defaultSecret($this->settings);
		}
		...
		public function canSetCWBranding() {
			if (SVWPSiteInfo::isCWServer()) {

				$bot_protect_accounts = SVAccount::accountsByType($this->settings, 'botprotect');
				if (sizeof($bot_protect_accounts) >= 1)
					return true;

				$bot_protect_accounts = SVAccount::accountsByPattern($this->settings, 'email', '/@cw_user\.com$/');
				if (sizeof($bot_protect_accounts) >= 1)
					return true;
			}
