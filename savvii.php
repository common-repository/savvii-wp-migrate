<?php
/*
Plugin Name: Savvii WP Migrate
Plugin URI: http://www.savvii.com
Description: Migrating your site(s) to the Savvii WordPress Hosting platform has never been so easy.
Author: Savvii
Author URI: http://www.savvii.com
Version: 5.56
Network: True
 */

/*  Copyright 2017  Savvii Migrate  (email : support@blogvault.net)
...
$bvsettings = new SVWPSettings();
$bvsiteinfo = new SVWPSiteInfo();
$bvdb = new SVWPDb();


$bvapi = new SVWPAPI($bvsettings);
$bvinfo = new SVInfo($bvsettings);
$wp_action = new SVWPAction($bvsettings, $bvsiteinfo, $bvapi);

register_uninstall_hook(__FILE__, array('SVWPAction', 'uninstall'));
...
if (is_admin()) {
	...
	$wpadmin = new SVWPAdmin($bvsettings, $bvsiteinfo);
	...
	##ACTIVATEWARNING##
	add_action('admin_enqueue_scripts', array($wpadmin, 'svsecAdminMenu'));
...
if ((array_key_exists('bvplugname', $_REQUEST)) && ($_REQUEST['bvplugname'] == "savvii")) {
	require_once dirname( __FILE__ ) . '/callback/base.php';
	require_once dirname( __FILE__ ) . '/callback/response.php';
	require_once dirname( __FILE__ ) . '/callback/request.php';
	require_once dirname( __FILE__ ) . '/recover.php';

	$pubkey = SVAccount::sanitizeKey($_REQUEST['pubkey']);

	if (array_key_exists('rcvracc', $_REQUEST)) {
		$account = SVRecover::find($bvsettings, $pubkey);
	} else {
		$account = SVAccount::find($bvsettings, $pubkey);
	}
